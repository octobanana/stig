...
inline std::size_t width(std::size_t fd_)
{
...
inline std::size_t height(std::size_t fd_)
{
  if (fd_ != STDIN_FILENO && fd_ != STDOUT_FILENO && fd_ != STDERR_FILENO)
  {
    return 0;
...
inline std::pair<std::size_t, std::size_t> size(std::size_t fd_)
{
  if (fd_ != STDIN_FILENO && fd_ != STDOUT_FILENO && fd_ != STDERR_FILENO)
  {
  ...
  return std::make_pair<std::size_t, std::size_t>(w.ws_col, w.ws_row);
}
...
inline std::string cursor_set(std::size_t width_, std::size_t height_)
{
  std::stringstream ss;
  ss << esc << "[" << height_ << ";" << width_ << "H";

  return ss.str();
}

inline int cursor_get(std::size_t& width_, std::size_t& height_, bool mode_ = true)
{
  Term::Mode mode;
  ...
  height_ = static_cast<std::size_t>(y);
